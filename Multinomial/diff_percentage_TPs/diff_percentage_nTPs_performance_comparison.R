## Performance comparison of different DA-test approaches
## Multinomial-distribution based simulation data with 
## different percentages of DA-OTUs
...
load("RAIDAreslist_diff_percentage_nTPs_small_effectsize_ns_5000_nsamples_25.RData")
load("DESeqreslist_diff_percentage_nTPs_small_effectsize_ns_5000_nsamples_25.RData")
load("DESeq2reslist_diff_percentage_nTPs_small_effectsize_ns_5000_nsamples_25.RData")
load("Omnibusreslist_diff_percentage_nTPs_small_effectsize_ns_5000_nsamples_25.RData")
load("MGSreslist_diff_percentage_nTPs_small_effectsize_ns_5000_nsamples_25.RData")
...
for(i in 1: length(RioNorm2reslist)){
  RioNorm2reslist[[i]] <- RioNorm2reslist[[i]]$result
}
...
print(metric_plot + ggtitle("Power plot under different percentage of TPs - All environments, Multi"))
...
metric_plot = ggplot(dfmeansd, aes(EffectSize, FDR, linetype = Method, color = Method, size=Method)) + geom_path(size = 1) +
  ...
  theme(legend.position = "bottom", legend.background = element_rect(color = "black", fill = "grey90", size = 0.1, linetype = "solid"))
print(metric_plot + ggtitle("FDR plot under different percentage of TPs - All environments, Multi"))
